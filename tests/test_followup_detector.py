...
def test_short_contextual_question_is_followup():
    ...
    assert is_followup("Cần bao nhiêu tiền?", has_history=True)
    assert is_followup("Thủ tục này mất bao lâu?", has_history=True)


def test_specific_procedure_starts_new_topic():
    assert not is_followup("Thủ tục cấp hộ chiếu cần gì?", has_history=True)
    assert not is_followup("Tôi muốn làm giấy khai sinh", has_history=True)
